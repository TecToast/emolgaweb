...
> =>
    defineEventHandler<EventHandlerRequest>(async (event) => {
        ...
        const result = await $fetch.raw(path, {
            headers: {
                UserID: user.id,
            },
        });
        result.headers.forEach((value, key) => appendHeader(event, key, value));
        return result._data;
    });

export const defineUnprotectedStaticContentRoute = <D>(): EventHandler<
    EventHandlerRequest,
    D
> =>
    defineEventHandler<EventHandlerRequest>(async (event) => {
