...
const errorMsg = computed(() => {
  if (invalidIndexes.value.length) {
    return `Die Ergebnisse ${data.value?.bo3 ? `im Kampf ${invalidIndexes.value.map(itm => Number.parseInt(itm) + 1).join(' & ')} ` : ''}scheinen nicht korrekt zu sein. Bitte überprüfe die Eingaben.`;
  }
  return null;
});
const modalDescription = computed(() => {
  ...
  return (
      ...
      allResultDatas.value.filter(rsData => Object.keys(rsData).length > 0).map(rsData =>
          Object.entries(rsData).map(
              ([player, mons]) =>
                  `${player}:\n${Object.entries(mons)
    ...
    <div class="flex flex-col items-center gap-2">
      ...
      <div v-if="data.bo3" class="my-4">
        <p class="px-2 xl:px-0 py-4">Wähle hier nacheinander die Kämpfe aus und trage die entsprechenden Daten ein. Lass
          den dritten Kampf leer, falls es nur zwei Kämpfe gab.</p>
        <UTabs
            v-model="selectedGame"
            :content="false"
            :items="[1,2,3].map((itm) => ({label: `Kampf ${itm}`, value: itm}))"></UTabs>
      </div>
      <p class="px-2 xl:px-0">
        Klicke alle Pokémon an, die in diesem Kampf verwendet wurden, und stelle
        dann jeweils die Kills und Deaths ein. Wenn du {{ data.bo3 ? 'mit allen Kämpfen ' : '' }}fertig bist, klicke auf
                ...
                <div class="w-full flex items-center gap-4">
                  ...
                  <UCheckbox
                    ...
                    "
                      ...
                      label="Besiegt?"
                      :model-value="
                      resultData[pdata.name]?.[mon.tlName]?.d ?? false
                    "
                      @update:model-value="(value) => resultData[pdata.name]![mon.tlName]!.d = !!value"
                  />
                </div>
              </template>
            </div>
