  ...
  <div class="grid place-content-center">
    <template v-if="league.loaded">
      <div v-if="league.leagues.length > 0" class="grid place-content-center">
        ...
        <div v-for="l in league.leagues" :key="l.name" class="flex flex-row gap-2">
          <img :src="l.logoUrl" alt=":)" class="rounded-full w-12">
          <RouterLink :to="'leagues/' + l.name"
                      class="text-lg text-center p-2 bg-cyan-600 hover:bg-cyan-800 transition duration-200 rounded">
            {{ convertLeagueIDToName(l.name) }}
          </RouterLink>
        </div>

      </div>
      <div v-else-if="league.error">Es konnte keine Verbindung zum Server hergestellt werden, versuche es später noch
        einmal.
      </div>
